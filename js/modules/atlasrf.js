/**
 * GridDown AtlasRF Integration Module
 * 
 * Connects to AtlasRF (atlasrf-v1) for real-time RF detection data:
...
const AtlasRFModule = (function() {
    ...
    const CONFIG = {
        ...
        mqttTopics: {
            tracks: 'atlasrf/tracks/#',
            trackUpdate: 'atlasrf/tracks/update',
            trackLost: 'atlasrf/tracks/lost',
            weather: 'atlasrf/weather/#',
            alerts: 'atlasrf/alerts',
            emergency: 'atlasrf/emergency'
        },
    ...
    function init() {
        if (initialized) {
            console.debug('AtlasRFModule already initialized');
            return;
        }
        
        // Create scoped event manager
        if (typeof EventManager !== 'undefined' && EventManager.createScopedManager) {
            atlasRFEvents = EventManager.createScopedManager(EventManager.SCOPES.ATLASRF || 'atlasrf');
        }
        
        // Load saved settings then auto-reconnect if previously connected
        loadSettings().then(() => {
            if (state.autoReconnect && state.host) {
                console.log(`AtlasRF: Auto-reconnecting to ${state.host}:${state.port} (method: ${state.connectionMethod})`);
                connect();
            }
        });
        
        initialized = true;
        console.log('AtlasRFModule initialized');
    }
    ...
    function destroy() {
        ...
        initialized = false;
        console.log('AtlasRFModule destroyed');
    }
    ...
    async function loadSettings() {
        try {
            ...
            const settings = await Storage.Settings.get('atlasrf_settings');
            if (settings) {
            ...
            // Same-origin detection: if GridDown is served from AtlasRF
            ...
            if (typeof window !== 'undefined' && window.location) {
                ...
                if (isPrivateIP && (!settings || !settings.host)) {
                    ...
                    console.log(`AtlasRF: Same-origin detected — using ${state.host}:${state.port}`);
                }
            }
        } catch (e) {
            console.warn('AtlasRF: Could not load settings:', e);
        }
    }
    ...
    async function saveSettings() {
        try {
            ...
            await Storage.Settings.set('atlasrf_settings', {
                host: state.host,
                port: state.port,
            ...
            console.warn('AtlasRF: Could not save settings:', e);
        }
    }
    ...
    function getWsUrl() {
        ...
        // Use the external integration endpoint — /ws requires AtlasRF
    ...
    async function connect(host = null, port = null, mqttPort = null) {
        if (state.connected || state.connecting) {
            console.log('AtlasRF: Already connected or connecting');
            return false;
        }
        ...
        console.log(`AtlasRF: Connecting to ${state.host}:${state.port} using method: ${method}`);
        ...
        if (method !== 'mqtt') {
            try {
                ...
                console.error('AtlasRF: Server not reachable:', e);
    ...
    async function connectAuto() {
        ...
        try {
            ...
            console.warn('AtlasRF: WebSocket failed, falling back to REST:', e);
            ...
            try {
                ...
                console.error('AtlasRF: REST polling failed:', restError);
                state.connecting = false;
    ...
    async function connectWithWebSocket() {
        try {
            ...
            console.error('AtlasRF: WebSocket connection failed:', e);
            state.connecting = false;
    ...
    async function connectWithMqtt() {
        try {
            ...
            console.error('AtlasRF: MQTT connection failed:', e);
            state.connecting = false;
    ...
    function connectWithRest() {
        try {
            ...
            console.error('AtlasRF: REST polling failed:', e);
            state.connecting = false;
    ...
    /**
     * Disconnect from AtlasRF server
     */
    function disconnect() {
        console.log('AtlasRF: Disconnecting...');
        ...
        if (state.mqttClient) {
            try {
                ...
                console.warn('AtlasRF: Error closing MQTT client:', e);
            }
    ...
    function connectWebSocket() {
        return new Promise((resolve, reject) => {
            ...
            console.log('AtlasRF: Connecting WebSocket to', wsUrl);
            ...
            const connectTimeout = setTimeout(() => {
                if (state.ws && state.ws.readyState === WebSocket.CONNECTING) {
                    console.warn('AtlasRF: WebSocket connection timeout, closing');
            ...
            state.ws.onopen = () => {
                ...
                console.log('AtlasRF: WebSocket connected');
            ...
            state.ws.onclose = (event) => {
                clearTimeout(connectTimeout);
                console.log('AtlasRF: WebSocket closed', event.code, event.reason);
                
                // Guard: if connection already fell back to REST, don't clobber state
                if (state.connectionMode === 'rest') {
                    console.log('AtlasRF: WebSocket onclose ignored — already using REST');
                    return;
                }
            ...
            state.ws.onerror = (error) => {
                console.error('AtlasRF: WebSocket error:', error);
    ...
    function scheduleWebSocketReconnect() {
        if (state.wsReconnectAttempts >= CONFIG.wsMaxReconnectAttempts) {
            console.log('AtlasRF: Max reconnect attempts reached, falling back to REST');
            startRestPolling();
            return;
        }
        ...
        console.log(`AtlasRF: Reconnecting in ${delay}ms (attempt ${state.wsReconnectAttempts})`);
        
        state.wsReconnectTimer = setTimeout(async () => {
            try {
                await connectWebSocket();
            } catch (e) {
                console.warn('AtlasRF: Reconnect failed:', e);
                scheduleWebSocketReconnect();
            }
        }, delay);
    }
     ...
     * only delivers events published *after* registration.  If AtlasRF
    ...
    async function seedInitialTracks() {
        try {
            ...
            console.warn('AtlasRF: Initial track seed failed (non-fatal):', e.message);
        }
    }
    ...
    function handleWebSocketMessage(data) {
        try {
            ...
            switch (message.type) {
                ...
                case 'track_batch':
                    // AtlasRF v3.3+ batches track updates for efficiency
                ...
                case 'fisb_update':
                    // AtlasRF emits FIS-B weather products as 'fisb_update'
                    handleFisBUpdate(message.data || message);
                    break;
                    
                case 'weather_conditions':
                    // Open-Meteo current conditions from AtlasRF
            ...
            console.warn('AtlasRF: Failed to parse WebSocket message:', e);
        }
    }
    
    /**
     * Handle batched track updates (AtlasRF v3.3+)
     * More efficient than individual updates for high-volume data
     */
    function handleTrackBatch(data) {
        if (!data || !data.tracks || !Array.isArray(data.tracks)) {
            console.warn('AtlasRF: Invalid track batch data');
            return;
        }
        
        const batchCount = data.count || data.tracks.length;
        console.debug(`AtlasRF: Processing batch of ${batchCount} tracks`);
    ...
    function handleServiceStatus(data) {
        ...
        if (data.service && data.status) {
            console.log(`AtlasRF: Service ${data.service} status: ${data.status}`);
        }
    }
    ...
    function handleServiceError(data) {
        ...
        console.warn('AtlasRF: Service error:', data);
    ...
    /**
     * Handle location updates from AtlasRF GPS
     */
    ...
    /**
     * Handle correlation events (AtlasRF Pro feature)
    ...
    function handleCorrelationEvent(eventType, data) {
        ...
        switch (eventType) {
            case 'correlation_new':
                console.log('AtlasRF: New correlation:', correlationId);
                if (correlationId) {
            ...
            case 'correlation_lost':
                console.log('AtlasRF: Correlation lost:', correlationId);
                if (correlationId) {
     ...
     * (AtlasRF and GridDown communicate over local network)
     */
    async function loadMqttLibrary() {
        ...
        try {
            ...
            console.log('AtlasRF: MQTT.js loaded from local bundle');
            mqttLibLoaded = true;
            return true;
        } catch (e) {
            console.warn('AtlasRF: Local MQTT.js not found, trying CDN...');
        }
        
        // Fallback to CDN (requires internet)
        try {
            await loadScript('https://unpkg.com/mqtt@5.3.4/dist/mqtt.min.js');
            console.log('AtlasRF: MQTT.js loaded from CDN');
            mqttLibLoaded = true;
            return true;
        } catch (e) {
    ...
    async function connectMqtt() {
        ...
        return new Promise((resolve, reject) => {
            ...
            console.log('AtlasRF: Connecting MQTT to', mqttUrl);
            ...
            state.mqttClient.on('connect', () => {
                clearTimeout(connectTimeout);
                console.log('AtlasRF: MQTT connected');
            ...
            state.mqttClient.on('close', () => {
                console.log('AtlasRF: MQTT connection closed');
            ...
            state.mqttClient.on('error', (error) => {
                console.error('AtlasRF: MQTT error:', error);
                clearTimeout(connectTimeout);
            ...
            state.mqttClient.on('offline', () => {
                console.warn('AtlasRF: MQTT client offline');
                if (state.connected && state.connectionMode === 'mqtt') {
    ...
    /**
     * Subscribe to AtlasRF MQTT topics
     */
    function subscribeMqttTopics() {
        ...
        const topics = [
            ...
            'atlasrf/correlation/#',   // Correlation events
            'atlasrf/status/#'         // Service status + GPS location
        ];
        
        topics.forEach(topic => {
            state.mqttClient.subscribe(topic, { qos: 0 }, (err) => {
                if (err) {
                    console.warn(`AtlasRF: Failed to subscribe to ${topic}:`, err);
                } else {
                    console.log(`AtlasRF: Subscribed to ${topic}`);
    ...
    function handleMqttMessage(topic, message) {
        try {
            ...
            console.warn('AtlasRF: Failed to parse MQTT message:', e, 'Topic:', topic);
        }
    }
    ...
    function scheduleMqttReconnect() {
        if (state.mqttReconnectAttempts >= CONFIG.mqttMaxReconnectAttempts) {
            console.log('AtlasRF: Max MQTT reconnect attempts reached');
        ...
        console.log(`AtlasRF: MQTT reconnecting in ${delay}ms (attempt ${state.mqttReconnectAttempts})`);
        
        state.mqttReconnectTimer = setTimeout(async () => {
            try {
                await connectMqtt();
            } catch (e) {
                console.warn('AtlasRF: MQTT reconnect failed:', e);
                scheduleMqttReconnect();
            }
        }, delay);
    }
    ...
    function startRestPolling() {
        ...
        console.log('AtlasRF: Starting REST polling');
    ...
    async function fetchAllTracks() {
        ...
        try {
            ...
            console.warn('AtlasRF: REST fetch failed:', e);
    ...
    function startHealthCheckInterval() {
        ...
        state.healthCheckTimer = setInterval(async () => {
            ...
            if (!health || health.status !== 'ok') {
                console.warn('AtlasRF: Health check failed');
    ...
    async function checkHealth(retries = 1, timeoutMs = 5000) {
        ...
        for (let attempt = 1; attempt <= attempts; attempt++) {
            try {
                ...
                if (!response.ok) {
                    if (attempt < attempts) {
                        console.warn(`AtlasRF: Health check attempt ${attempt}/${attempts} got HTTP ${response.status}, retrying...`);
                        continue;
                    }
                    return null;
                }
                ...
                if (attempt > 1) {
                    console.log(`AtlasRF: Health check succeeded on attempt ${attempt}/${attempts}`);
                }
                
                return health;
            } catch (e) {
                if (attempt < attempts) {
                    console.warn(`AtlasRF: Health check attempt ${attempt}/${attempts} failed (${e.message}), retrying...`);
                } else {
                    console.warn('AtlasRF: Health check error:', e);
                    return null;
                }
            }
        }
    ...
    /**
     * Handle FIS-B weather product push from AtlasRF WebSocket
    ...
    /**
     * Handle Open-Meteo current weather conditions from AtlasRF
    ...
    async function fetchFisBWeather() {
        ...
        try {
            ...
            console.warn('AtlasRF: Failed to fetch FIS-B weather:', e);
            return null;
        }
    }
    ...
    /**
     * Render AtlasRF tracks on the map canvas
    ...
    /**
     * Render the AtlasRF station location on the map
    ...
    function renderStationMarker(ctx, width, height, latLonToPixel, zoom) {
        ...
        if (zoom >= 8) {
            ...
            ctx.strokeStyle = 'rgba(0,0,0,0.7)';
            ctx.lineWidth = 2;
            ctx.strokeText('AtlasRF', x, y + 22);
            ctx.fillText('AtlasRF', x, y + 22);
        }
    ...
    function emitEvent(eventName, data) {
        if (typeof Events !== 'undefined') {
            Events.emit(`atlasrf:${eventName}`, data);
        }
    }
...
// Register globally
window.AtlasRFModule = AtlasRFModule;
